...
def parse_rt_f(rt_f, indir):
    ...
    :return:
    """
    data = pd.read_csv(rt_f, header=None, sep='\t')
    new_files = [os.path.join(indir, str(_)) for _ in data.iloc[:, 0]]
    return dict(zip(new_files,
                    data.iloc[:, 1]))
...
def split_video_trans(indir, date, rt_dict, raw_stime, raw_etime, fname, cov_time=True, keep_intermediate=False):
    ...
    video1, video_stime1, video_duration1 = _sub_split(indir, date, s_h,rt_dict, raw_stime, raw_etime, cov_time=cov_time)
    video2, video_stime2, video_duration2 = _sub_split(indir, date, e_h,rt_dict, raw_stime, raw_etime, cov_time=cov_time)
...
def main(indir, odir, rt_dict, metadata, dry_run=False,keep_intermediate=False):
    """
...
if __name__ == '__main__':
    ...
    odir = '/home/liaoth/data2/project/VD/data2/splitted2'
    os.makedirs(odir, exist_ok=True)
    rt_f = '/home/liaoth/data2/project/VD/data2/raw/all_rt.txt'
    rt_dict = parse_rt_f(rt_f, indir)

    metadata = pd.read_excel('/home/liaoth/data2/project/VD/data2/split_video_2.xlsx', dtype=str)
    main(indir, odir, rt_dict, metadata, dry_run=False)
